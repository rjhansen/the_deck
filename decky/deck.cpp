...
namespace The_Deck
{
    ...
    void Deck::triple_cut()
    {
        ...
        if (first_joker == deck.cend()) // Test if there are not enough jokers in the deck
            throw logic_error("No jokers found while trying to perform a triple cut. We need two.");

        auto second_joker = find_if(first_joker + 1,
            deck.end(),
            [](const auto& card) {
                return (card.RANK == Card::Rank::JOKER_A || card.RANK == Card::Rank::JOKER_B);
            });
        if (second_joker == deck.cend())
            throw logic_error("Only one joker found while trying to perform a triple cut. We need two.");
        ...
        copy(to_first_joker.cbegin(), to_first_joker.cend(), back_inserter(after_second_joker));
